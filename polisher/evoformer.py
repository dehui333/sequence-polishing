...
class Evoformer(nn.Module):
    def __init__(self, msa_embedding_dim, heads, num_blocks, p_keep_lowest):
        ...
        self.blocks = nn.Sequential(*[
            EvoformerBlock(msa_embedding_dim, heads, prob_fn(i + 1))
...
class EvoformerBlock(nn.Module):
    def __init__(self, msa_embedding_dim, heads, p_keep):
        ...
        self.msa_col_att = MSAGatedAttention('column', msa_embedding_dim,
                                             heads)
        self.p_keep = p_keep
        self.msa_transition = Transition(msa_embedding_dim,
                                         projection_factor=4)
...
class Transition(nn.Module):
    def __init__(self, embedding_dim, projection_factor=4):
        ...
        self.linear1 = nn.Linear(embedding_dim,
                                 projection_factor * embedding_dim)
        self.linear2 = nn.Linear(projection_factor * embedding_dim,
                                 embedding_dim)
