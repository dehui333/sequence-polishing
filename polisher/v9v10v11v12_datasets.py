...
class StorageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # find the invalid value encountered in true_divide
        warnings.filterwarnings("error")
        try:
            np.nan_to_num(group['stats'][p]/np.sum(group['stats'][p],axis=0))
        except:
            print("sum is 0 here!", self.filenames[f_idx], g, p)
