...
from v17_roko_data_module import RokoDataModule
import sys
import logging
...
logging.basicConfig(filename='/raid/scratch/wuws/docker/v12_weighted_mapq/sequence-polishing/polisher/runs/v17_debug.log', encoding='utf-8', level=logging.INFO)
...
class Polisher(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        reads, labels, pos_stat, cov, f_name, g, p = batch
        ...
        seq_logits, mask_logits = self.forward_train(reads, mask, pos_stat, cov) # logits = B S C (B x S x 5), y = B S, attn_out = N_masked x 5, before_masking = N_masked
        
        seq_logits = seq_logits.transpose(1,2) # logits = B C S (B x 5 x S)
        ...
        train_acc_batch = self.train_accuracy(seq_logits, labels) # logits B C S (B x 5 x S), labels B x S
        self.log('train_acc', train_acc_batch)

        if self. current_epoch >= 5 and train_acc_batch < 1: # add in the epoch condition later (if epoch > 10 or something)
            for b in range(self.trainer.datamodule.batch_size): # should be logged batch_size times
                train_acc_example = accuracy(seq_logits[b].transpose(0,1), labels[b]) # S x 5 and S
                logging.info('At epoch %d, train_acc_batch = %f, train_acc_example = %f, f_name: %s, g: %s, p: %d \n', self.current_epoch, train_acc_batch, train_acc_example, f_name[b], g[b], p[b])
        #sys.exit()
...
def get_trainer_defaults() -> Dict[str, Any]:
    ...
    trainer_defaults = {
        ...
        'logger': WandbLogger(project='hyper_para_tuning', log_model='all', save_dir=os.getcwd()), # weights and biases
        'strategy': 'ddp' # 'strategy' parameter requires pytorch lightning v1.5 and above
    }
