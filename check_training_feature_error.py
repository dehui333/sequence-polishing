...
class Alignment:
    def __init__(self, align, start, end, name):
        self.align = align
        self.start = start
        self.end = end
        self.name = name
...
def get_aligns_with_name(bam, ref_name=None, start=0, end=None):
    filtered = []
    ...
    with pysam.AlignmentFile(bam, 'rb', index_filename=bam + '.bai') as f:
        for r in f.fetch(ref_name, start, end):
            ...
            if not r.is_unmapped and not r.is_secondary:
                filtered.append(Alignment(r, r.reference_start, r.reference_end, r.query_name))
...
def list_align(alignments):
    for i in range(len(alignments)):
        print(alignments[i].start,alignments[i].end)
